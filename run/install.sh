...
script_path=$(pwd)
chmod +x "$script_path/probat_run.sh"
user_dir=$(eval echo ~${SUDO_USER})
if grep -q "function $function_name" "$user_dir/.bashrc"; then
  sed -i "s|function $function_name() {.*|function $function_name() { $script_path/probat_run.sh \"$script_path\" \"\$*\"; }|" "$user_dir/.bashrc"
  echo "PROBAT atualizado com sucesso. Agora você pode executar '$function_name' em qualquer lugar."
else
  echo "function $function_name() { $script_path/probat_run.sh \"$script_path\" \"\$*\"; }" >> "$user_dir/.bashrc"
